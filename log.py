...
with st.expander("Components to build:"):
  st.markdown(":pencil2:   Quick summary - display time logged in Jira for the day/week.")
  st.markdown(":pencil2:   Required: date, task (drop down), start datetime, duration (or end time);Optional: comments")
  st.markdown(":pencil2:   Commit button")
  st.markdown(":pencil2:   Button to pull in calendar events (user input for time period) - create dataframe with drop down for each item to log event to Jira")
...
with st.expander("Functionalities:"):
  ...
  st.markdown(":firecracker:   Pull events from Google Calendar")
  st.markdown("&nbsp;&nbsp;:boom:   Only show if config details present")
